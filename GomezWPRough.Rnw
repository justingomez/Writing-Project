...
Before we begin fitting models according to specific methods, let's discuss the models that we will consider fitting with the different methods. Our data set consists of the eight previously discussed games statistics. To understand the relationships present between the quantitivative variables, it may be helpful to examine their correlations in Figure~\ref{fig:corr}.\\
